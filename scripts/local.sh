...
if ! toml --help &>/dev/null; then
    echo "toml-cli is not installed. Installing..."
    if ! pip install toml-cli; then
        echo "Failed to install toml-cli. Please make sure pip is installed."
        exit 1
    fi
fi
...
ADDRESS1=$(sawd keys show tieubaoca01 --output json | jq '.address')
...
cat $HOME/.saw/config/genesis.json | jq '.app_state["nft"]["classes"] += [{"id": "SARallyBell", "name": "SA Rally Bell", "symbol": "SARB"}]' > $HOME/.saw/config/tmp_genesis.json && mv $HOME/.saw/config/tmp_genesis.json $HOME/.saw/config/genesis.json
cat $HOME/.saw/config/genesis.json | jq '.app_state["nft"]["classes"] += [{"id": "SAHero", "name": "SA Hero", "symbol": "SAH"}]' > $HOME/.saw/config/tmp_genesis.json && mv $HOME/.saw/config/tmp_genesis.json $HOME/.saw/config/genesis.json
cat $HOME/.saw/config/genesis.json | jq '.app_state["nft"]["entries"] += [{"owner": '$ADDRESS1', "nfts": [{"classId": "SARallyBell", "id": "SARB-0001"}]}]' > $HOME/.saw/config/tmp_genesis.json && mv $HOME/.saw/config/tmp_genesis.json $HOME/.saw/config/genesis.json
cat $HOME/.saw/config/genesis.json | jq '.app_state["nft"]["entries"] += [{"owner": '$ADDRESS1', "nfts": [{"classId": "SARallyBell", "id": "SARB-0002"}]}]' > $HOME/.saw/config/tmp_genesis.json && mv $HOME/.saw/config/tmp_genesis.json $HOME/.saw/config/genesis.json
cat $HOME/.saw/config/genesis.json | jq '.app_state["nft"]["entries"] += [{"owner": '$ADDRESS1', "nfts": [{"classId": "SAHero", "id": "SAH-0001"}]}]' > $HOME/.saw/config/tmp_genesis.json && mv $HOME/.saw/config/tmp_genesis.json $HOME/.saw/config/genesis.json
cat $HOME/.saw/config/genesis.json | jq '.app_state["nft"]["entries"] += [{"owner": '$ADDRESS1', "nfts": [{"classId": "SAHero", "id": "SAH-0002"}]}]' > $HOME/.saw/config/tmp_genesis.json && mv $HOME/.saw/config/tmp_genesis.json $HOME/.saw/config/genesis.json
